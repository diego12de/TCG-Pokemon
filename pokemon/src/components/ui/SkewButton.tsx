...
export const SkewButton = React.forwardRef<HTMLButtonElement, SkewButtonProps>(
  ({ label, href, variant = "primary", size = "md", className, ...props }, ref) => {
    const baseStyles = "relative inline-flex items-center justify-center font-body font-semibold uppercase tracking-wider transition-all duration-300 touch-manipulation overflow-hidden";
    ...
    const content = (
      <>
        ...
        <span className={cn("relative z-10 flex items-center gap-2", innerStyles)}>
          {variant === "discord" && (
          ...
          {label}
          {variant === "ghost" && <span className="transition-transform duration-300 group-hover:translate-x-1">→</span>}
        </span>
      </>
    );
    ...
    if (href) {
      return (
        <a href={href} className={cn(baseStyles, sizeStyles[size], variantStyles[variant], "group", className)}>
          {content}
        </a>
      );
    }
    ...
    return (
      <button ref={ref} className={cn(baseStyles, sizeStyles[size], variantStyles[variant], "group", className)} {...props}>
        {content}
      </button>
    );
  }
