...
export function Navbar({ onViewChange }: { onViewChange: (view: "home" | "singles") => void }) {
    ...
    >
      <div className="max-w-7xl mx-auto h-full px-4 sm:px-6 lg:px-8 flex items-center justify-between">
            ...
            {megaMenuOpen && (
              ...
              >
                ...
                <div>
                  ...
                  <ul className="space-y-3">
                    <li><button onClick={() => onViewChange("singles")} className="text-sm font-medium hover:text-accent transition-colors">Singles</button></li>
                    <li><button onClick={() => onViewChange("singles")} className="text-sm font-medium hover:text-accent transition-colors">Producto Sellado</button></li>
                    <li><button onClick={() => onViewChange("singles")} className="text-sm font-medium hover:text-accent transition-colors">Accesorios</button></li>
                    <li><button onClick={() => onViewChange("singles")} className="text-sm font-medium hover:text-accent transition-colors text-gold">Outlet & Ofertas</button></li>
                  </ul>
                </div>
              </div>
            )}
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            className="p-2 text-text-muted hover:text-white transition-colors relative touch-manipulation min-w-[44px] min-h-[44px] flex items-center justify-center" 
            aria-label="Carrito"
          ...
          <div className="absolute top-0 right-0 bottom-0 w-[300px] max-w-[80vw] bg-surface border-l border-border shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <nav className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
              <button 
                ...
                className="text-left text-lg font-heading font-semibold hover:text-accent"
              >
                Inicio
              </button>
              <div className="space-y-4">
                ...
                {siteConfig.games.map(game => (
                  <button 
                    ...
                    className="block w-full text-left text-base font-medium hover:text-accent"
                  >
                    {game}
                  </button>
                ))}
